...
class MoneySystemsTestCase(unittest.TestCase):
    ...
    def test_sample_input(self):
        ...
        target_value = 10
        money_sys = MoneySystem(coins)
        self.assertEqual(10, money_sys.count_constructions(target_value))

    def test_simple_example(self):
        coins = [2, 10]
        target_value = 18
        money_sys = MoneySystem(coins)
        self.assertEqual(2, money_sys.count_constructions(target_value))
    ...
    def test_one_coin_that_is_a_factor_of_the_target_value_can_only_produce_the_value_in_one_way(self):
        ...
        for k in range(1, 10):
            money_sys = MoneySystem(coins)
            self.assertEqual(1, money_sys.count_constructions(k * coins[0]))
